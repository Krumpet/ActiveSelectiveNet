...
for i in range(NUMBER_OF_RUNS):
    ...
    while cur_train_size <= x_total.shape[0]:
        ...
        # set file name
        baseline_file_name = "SRbaseline_round_{}_trainSize_{}.h5".format(i, cur_train_size)
        ...
        x_train_baseline, y_train_baseline = x_total[cur_train_indexes], y_total[cur_train_indexes]
        cur_test_indexes = all_indexes[np.logical_not(np.isin(all_indexes, cur_train_indexes))]
        x_test_baseline, y_test_baseline = x_total[cur_test_indexes], y_total[cur_test_indexes]
        ...
        sio.savemat('test_result/test_result_{}.mat'.format(baseline_file_name[:-3]), {'metrics_names': metrics_names,
                                                                           'scores': scores})
        _, y_pred = baseline_net.predict()
        y_confidence = np.amax(y_pred, 1)
